...
// Функция инициализации – вызывается при открытии модалки
function initWheel() {
    let container = document.getElementById('wheelContainer');
    if (!container) {
        console.error('Не найден контейнер #wheelContainer');
        return;
    }
    ...
    if (w === 0 || h === 0) {
        console.warn('Контейнер скрыт, откладываем инициализацию');
        return;
    }
...
function setup() {
    // Ничего не делаем – ждём вызова initWheel()
}
...
// Делаем initWheel доступной глобально
